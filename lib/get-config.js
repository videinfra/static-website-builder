...
/**
 * Returns builder and project specific task configurations combined
...
function getConfig () {
    if (!taskConfig) {
        const builderConfigPath = path.getBuilderPath('config/task-config.js');
        taskConfig = itterateConfig(require(builderConfigPath), mergeConfigMode);

        const builderConfigFolder = process.env.BUILDER_CONFIG_PATH || 'config';
        const projectConfigPath = path.getProjectPath(builderConfigFolder, 'task-config.js');

        // Load project specific task-config.js
        if (builderConfigPath !== projectConfigPath && fs.existsSync(projectConfigPath)) {
            const projectTaskConfig = itterateConfig(require(projectConfigPath), mergeConfigMode);
            taskConfig = merge(taskConfig, projectTaskConfig);
        }

        // Marge plugins into the config, each plugin is just a config
        const plugins = taskConfig.plugins;
        if (plugins) {
            taskConfig.plugins = null;

            each(plugins, (plugin) => {
                taskConfig = merge(taskConfig, plugin);
            });
        }
